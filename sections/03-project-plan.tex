...
Our team adopted a hybrid Waterfall + Agile project management
approach for this project. This methodology provided us with both the
structured framework of Waterfall for critical path activities and
the flexibility of Agile for iterative development and testing. This
hybrid approach was particularly well-suited for this project because:

\begin{enumerate}
  \item The semantic segmentation optimization had clearly defined
    phases (pipeline architecture design, implementation, testing)
    that benefited from Waterfall planning
  \item The technical nature of implementing pipelined processing,
    DPU scheduling, and optimizing algorithms required adaptive
    iterations that benefited from Agile sprints
  \item Working with specialized hardware (Kria Board KV260) required
    careful planning of resource allocation and sequential DPU access scheduling
\end{enumerate}

For project tracking, the team utilized the following tools:

\begin{itemize}
  \item \textbf{GitHub}: Primary code repository for version control,
    documentation, and collaboration. Our client also had access to
    this repository to track progress in real-time.
  \item \textbf{Telegram}: Main communication channel with our client
    and previous years' team members for quick updates and questions.
\end{itemize}

Weekly team meetings were held to review sprint progress, address
blockers, and plan upcoming work. Monthly meetings with the client
ensured alignment with project goals and requirements.

\section{Task Decomposition}\label{sec:task-decomposition}

Our project involved optimizing the semantic segmentation U-Net
algorithm by implementing a pipelined architecture that leveraged
multi-threaded CPU processing alongside sequential DPU execution\@.
The key objective was to achieve target throughput of 60 FPS for
real-time medical monitoring. The following tasks and subtasks were identified:
...
These tasks were further broken down into sprint activities with
specific team members assigned based on their expertise.

\section{Project Milestones, Metrics, and Evaluation
Criteria}\label{sec:milestones}

The following key milestones were identified for the project, along
with their associated metrics and evaluation criteria:
...
For each milestone, our team tracked progress using the following
...
The project spanned approximately 16 weeks, with work organized into
sprints. The project timeline followed a structured approach with
major milestones and deliverable dates:
...
The critical path for this project follows the U-Net model
optimization and pipelining, implementation of the eye-tracking
components, integration of the processing pipeline, and final
...
For risks with probability exceeding 30\%, our team developed
detailed contingency plans, including alternative implementation
approaches and resource reallocation strategies.
