...
\item Working with specialized hardware (AMD Kria KV260 board) required careful planning of resource allocation and sequential DPU access scheduling
\end{enumerate}
...
Our project involved optimizing the semantic segmentation U-Net algorithm by implementing a pipelined architecture that leveraged multithreaded CPU processing alongside sequential DPU execution\@. The key objective was to achieve target throughput of 60 FPS for real-time medical monitoring. The following tasks and subtasks were identified:
...
\begin{itemize}
\item Subtask 2.1: Implement image preprocessing using semantic segmentation
\item Subtask 2.2: Implement eye tracking algorithm with preprocessed images
...
\item \textbf{Metrics}: Successful multithreaded operation for CPU-based preprocessing and postprocessing
