...
\begin{enumerate}
  ...
  \item \textbf{Previous Project Iterations} at Iowa State University
    (SDMAY25-01~\cite{sdmay25-01}, SDDEC24-05~\cite{sddec24-05}, % chktex 8
    SDDEC23-02~\cite{sddec23-02}, SDDEC22-14~\cite{sddec22-14}) % chktex 8
    developed eye tracking systems for assistive technology. The most
    recent iteration (SDDEC24-05) implemented a standard U-Net % chktex 8
    architecture on the Kria KV260 board with high accuracy (99.8\% IoU)
    but could only process a single frame every 160ms, which is
    insufficient for real-time application needs.
