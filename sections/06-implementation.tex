...
\begin{itemize}
	\item \textbf{Mean Total Latency}: 534.65 ms ($\pm$ 2.26 ms)
	\item \textbf{Mean DPU Time}: 474.16 ms ($\pm$ 0.04 ms)
	\item \textbf{Mean Preprocess}: 31.78 ms ($\pm$ 1.21 ms)
	\item \textbf{Mean Postprocess}: 28.71 ms ($\pm$ 1.06 ms)
	\item \textbf{Throughput}: 1.87 FPS
	\item \textbf{Memory Usage}: 42.99 MB
\end{itemize}

Single model provides stable baseline with low variance. DPU execution
dominates (88.7\% of latency), with pre/postprocessing contributing approximately 11\%.
...
\begin{itemize}
	\item \textbf{Mean Total Latency}: 4918.27 ms ($\pm$ 1.63 ms)
	\item \textbf{Mean DPU Time}: 4859.85 ms ($\pm$ 1.61 ms)
	\item \textbf{Mean Preprocess}: 30.79 ms ($\pm$ 0.31 ms)
	\item \textbf{Mean Postprocess}: 27.63 ms ($\pm$ 0.03 ms)
	\item \textbf{Throughput}: 0.20 FPS
	\item \textbf{Memory Usage}: 134.00 MB
\end{itemize}
...
Split model incurs 820\% higher latency (9.20x slower) from segment coordination,
...
\begin{itemize}
	\item \textbf{Parallelization Overhead}: Naive algorithm division
	      introduces approximately 4384ms overhead from segment coordination,
	      data transfer, and Vitis AI compiler scaling corrections
	\item \textbf{Memory Efficiency}: Single model: 42.99MB vs split model:
	      134.00MB due to intermediate tensor storage between segments
	\item \textbf{Performance Ratio}: Single model is 9.20x faster than
	      split model (DPU time ratio: 0.10x)
	\item \textbf{Throughput Comparison}: Single model: 1.87 FPS vs split
	      model: 0.20 FPS
\end{itemize}
...
benefits of pipelining, motivating alternative optimization strategies focused
on sequential execution with optimized CPU-DPU overlap.
