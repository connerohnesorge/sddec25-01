...
Our project has completed comprehensive performance evaluation of
both single-model and split-model architectures on the AMD Kria KV260
development board. The implementation follows a structured approach
focusing on the critical path items while maintaining flexibility for
...
We have conducted extensive benchmarking of both single-model and
split-model implementations to evaluate the trade-offs between
different parallelization strategies.

\subsubsection{Single Model
Performance}\label{subsubsec:single-model-performance}

The baseline U-Net implementation running as a single unified model
demonstrates the following performance characteristics:
...
The single model implementation provides a stable baseline with low
variance in processing times, indicating consistent performance
across frames. The dominant processing time is spent in DPU execution
(89.6\% of total latency), with preprocessing and postprocessing
contributing approximately 10\% of overall latency.

\subsubsection{Split Model Performance (4
Segments)}\label{subsubsec:split-model-performance}

The parallelized implementation dividing the U-Net algorithm into
four segments demonstrates the following characteristics:
...
The split model approach incurs additional overhead from thread
coordination and data transfer between segments, resulting in 48.2\%
higher total latency compared to the single model. The increased
variance ($\pm$ 3.39 ms vs. $\pm$ 0.26 ms) indicates synchronization
...
\begin{itemize}
  \item \textbf{Parallelization Overhead}: The split model
    demonstrates that naive algorithm division introduces significant
    overhead (255 ms increase) due to thread synchronization and
    inter-segment data transfer
  \item \textbf{Memory Efficiency}: Single model maintains a lower
    memory footprint (45 MB) when compared to split model inference
    (111.01 MB). This increase in memory footprint is caused by the
    intermediate storage of tensors needed as inputs for further UNet segments.
\end{itemize}

\subsection{Algorithm Analysis and Division}\label{subsec:algorithm-analysis}

The U-Net semantic segmentation algorithm (Figure~\ref{fig:unet}) has
been thoroughly analyzed for pipelined processing
optimization~\cite{zhao2023parallel}:
...
into segments for parallel processing, we utilized
Netron~\cite{netron}, an open-source neural network visualization
tool. Netron provides comprehensive visualization of ONNX model
graphs, enabling detailed inspection of the network architecture including:
...
Using Netron's interactive graph visualization, we identified
strategic nodes in the U-Net encoder-decoder architecture where the
model could be divided into four segments while maintaining the skip
connection pathways essential for semantic segmentation accuracy.
This analysis informed the split model implementation evaluated in
Section~\ref{subsubsec:split-model-performance}, where intermediate
tensors between segments are stored and passed to subsequent processing stages.

\subsubsection{Model Extraction with ONNX
Utilities}\label{subsubsec:onnx-model-extraction}

After identifying the appropriate split points using Netron
visualization, we utilized the ONNX utilities library~\cite{onnx2023}
to programmatically extract sub-models from the original U-Net
architecture. The \texttt{onnx.utils.extract\_model} function enables
precise extraction of model segments by specifying input and output
tensor names:
...
The extraction process allowed us to systematically divide the U-Net
architecture into four independent segments, each represented as a
separate ONNX model file. This modular approach enables flexible
deployment strategies, including the split model implementation
evaluated in Section~\ref{subsubsec:split-model-performance}. Each
extracted segment maintains the precise computational semantics of
the corresponding portion of the original model, ensuring that when
executed sequentially with proper intermediate tensor passing, the
split model produces identical results to the unified architecture.
...
This extraction methodology provided the foundation for our
multi-segment deployment experiments, enabling direct comparison
between single-model and split-model architectures on the Kria KV260
hardware platform.
...
The U-Net semantic segmentation model training process underwent
comprehensive redesign to improve training efficiency, model
performance, and deployment readiness. Our enhanced training
infrastructure leverages cloud-based GPU acceleration through
Modal.com~\cite{modal2024} with PyTorch~\cite{pytorch2023},
implementing sophisticated loss functions, comprehensive experiment
tracking, and automated model export for the deployment pipeline.

\subsubsection{Cloud-Based Training Environment}

The training infrastructure utilizes Modal.com for scalable,
reproducible GPU training with the following specifications:
...
The training implementation employs a sophisticated multi-component
loss function that combines three complementary objectives to achieve
...
Early training (epochs 1--125) emphasizes surface loss to establish
correct boundary topology, while later training focuses on dice loss
to optimize overall segmentation quality. This two-phase approach
prevents the model from learning topologically incorrect solutions
that would be difficult to correct in later training.

\subsection{GPU-Optimized Training
Pipeline}\label{subsec:gpu-optimized-training}

The training implementation prioritizes GPU-native operations to
minimize performance overhead from CPU-GPU data transfers:
...
The training pipeline implements comprehensive augmentation to
...
The optimized training infrastructure achieved substantial
...
The performance gains result from: (1) GPU-accelerated training
eliminating CPU-GPU synchronization overhead, (2) sophisticated loss
function improving convergence quality, (3) optimized data pipeline
maximizing GPU utilization, and (4) comprehensive augmentation
enabling efficient learning from limited data.

\subsection{Experiment Tracking and Artifact
Management}\label{subsec:experiment-tracking}

Training experiments are tracked using MLflow~\cite{mlflow2023}
integrated with GitLab's ML model registry, providing comprehensive
logging of training artifacts, performance metrics, and automated
model versioning for the deployment pipeline.

\subsubsection{Comprehensive Metadata Logging}

The training system logs extensive metadata for complete experiment
reproducibility:
...
Due to current limitations in GitLab's MLflow backend (inability to
properly visualize step-based metrics), the training system
implements a pragmatic workaround:
...
This approach trades run count for compatibility with GitLab's
current MLflow implementation, ensuring experiment tracking
functionality while awaiting platform improvements.

\subsection{GPU Metrics Methodology}\label{subsec:gpu-metrics-methodology}

Following best practices for GPU performance
measurement~\cite{elvinger2025gpu}, our training implementation
computes all performance metrics directly within GPU kernels rather
than relying on CPU-side calculations. This approach provides several
advantages:
...
The methodology aligns with recent research demonstrating that
conventional ``GPU utilization'' metrics measured from the CPU side
are insufficient for understanding true GPU resource
usage~\cite{elvinger2025gpu}. By measuring performance at the kernel
level using device-side counters, we achieve more accurate
characterization of computational efficiency and identify
optimization opportunities that would be invisible to host-side profiling tools.

\subsection{ONNX Model Export and Deployment
Integration}\label{subsec:onnx-export}

The training pipeline automatically exports trained models to ONNX
format for seamless integration with the Vitis AI quantization and
deployment workflow:
...
This automated export eliminates manual conversion steps and ensures
training outputs directly feed into the FPGA deployment pipeline,
reducing potential for errors in model transfer between training and
deployment environments.

\subsection{Training Dataset and Validation}\label{subsec:training-dataset}

The model training process utilizes the OpenEDS (Open Eye
Dataset)~\cite{openeds2019} with automated acquisition and
