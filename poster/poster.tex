% VisionAssist Senior Design Poster
% Iowa State University SDDEC25-01
% A0 Portrait Format Poster

\documentclass[25pt, a0paper, portrait]{tikzposter}

% Essential packages
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{microtype}
\usepackage{booktabs}

% TikZ libraries for coordinate calculations
\usetikzlibrary{calc}

% Bibliography setup - using relative path to main project references
\usepackage[backend=biber, style=ieee, sorting=none]{biblatex}
\addbibresource{../references.bib}

% Load poster theme configuration
\makeatletter
\input{poster-config.tex}
\makeatother

% Document metadata
\title{}
\author{Iowa State University Senior Design Team SDDEC25--01}
\institute{Department of Electrical and Computer Engineering}

% Adjust title styling for readability
\settitle{
  \centering
  \vbox{
    \centering
    \color{titlefgcolor}
    {\bfseries \Huge \@title \par}
    \vspace*{0.25em}
    {\LARGE \textbf{Senior Design Project SDDEC25--01} \par}
    \vspace*{0.15em}
    {\Large Conner Ohnesorge (\href{https://github.com/connerohnesorge}{@connerohnesorge}), Tyler Schaefer (\href{https://github.com/TSSchaef}{@TSSchaef}),\par}
    \vspace*{0.05em}
    {\Large Aidan Perry (\href{https://github.com/priedieux}{@priedieux}), Joey Metzen (\href{https://github.com/jjmetzen}{@jjmetzen}) \par}
    \vspace*{0.1em}
    {\Large Iowa State University \par}
    \vspace*{0.05em}
    {\large Department of Electrical and Computer Engineering}
  }
}

\begin{document}

\maketitle

% Two-column layout for better content organization
\begin{columns}
	\column{0.5}

	% LEFT COLUMN

	\block{Problem \& Approach}{
		\input{sections/introduction.tex}
	}

	\block{System Architecture}{
		\input{sections/architecture.tex}
	}

	\block{Hardware \& AI Model}{
		\input{sections/methodology.tex}
	}

	\column{0.5}

	% RIGHT COLUMN

	\block{Performance \& Validation}{
		\input{sections/results.tex}
	}

	\block{Impact \& Next Steps}{
		\input{sections/conclusion.tex}
	}

\end{columns}

% Bottom section spanning full width
\block{Acknowledgments}{
	\input{sections/acknowledgments.tex}
}

% Bibliography section at bottom
\block{References}{
	\printbibliography[heading=none]
}

\end{document}
