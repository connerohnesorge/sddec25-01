...
def load_model(checkpoint_path: str, model_size: str = "small", device: str = "cuda"):
    """Load trained NSA model from checkpoint."""
    model = create_nsa_pupil_seg(size=model_size, in_channels=1, num_classes=2)

    if checkpoint_path:
        checkpoint = torch.load(checkpoint_path, map_location=device, weights_only=False)
        model.load_state_dict(checkpoint["model_state_dict"])
        print(f"Loaded model from {checkpoint_path}")
        print(f"Best IoU: {checkpoint.get('valid_iou', 'N/A'):.4f}")
...
def preprocess_image(image_path: str, target_size: tuple = (400, 640)):
...
def predict(model, img_tensor, device: str = "cuda"):
...
def visualize_results(
    ...
    axes[1].set_title("Pupil Segmentation")
    axes[1].axis("off")

    # Overlay
    overlay = np.stack([original] * 3, axis=-1).astype(np.float32) / 255.0
...
def demo_synthetic():
    ...
    print("\n" + "=" * 60)
    print("NSA Pupil Segmentation Demo (Synthetic Image)")
    print("=" * 60)
    ...
    model = create_nsa_pupil_seg(size="small").to(device)
    model.eval()
    ...
    img = np.ones((H, W), dtype=np.float32) * 200

    # Add iris (darker circle)
    cy, cx = H // 2, W // 2
    iris_radius = 100
    Y, X = np.ogrid[:H, :W]
    iris_mask = (X - cx) ** 2 + (Y - cy) ** 2 <= iris_radius ** 2
    img[iris_mask] = 80

    # Add pupil (even darker circle)
    pupil_radius = 40
    pupil_mask = (X - cx) ** 2 + (Y - cy) ** 2 <= pupil_radius ** 2
    img[pupil_mask] = 20
    ...
    print(f"\nResults:")
    print(f"  Predicted pupil pixels: {(pred == 1).sum():,}")
    print(f"  Ground truth pupil pixels: {gt.sum():,}")
    print(f"  IoU: {iou:.4f}")

    # Visualize
    visualize_results(img, pred, save_path="nsa_demo_synthetic.png", show=True)
...
def main():
    ...
    if args.image is None:
        ...
        img_tensor, original = preprocess_image(args.image)
        prediction = predict(model, img_tensor, device)

        output_path = args.output or f"nsa_output_{args.image.split('/')[-1]}"
        visualize_results(original, prediction, save_path=output_path, show=not args.no_show)
