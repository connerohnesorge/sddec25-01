...
def test_token_compression():
    ...
    compression = TokenCompression(dim=dim, block_size=block_size, stride=stride)

    k = torch.randn(batch_size, seq_len, dim)
    v = torch.randn(batch_size, seq_len, dim)

    k_cmp, v_cmp = compression(k, v, (H, W))

    print(f"  Input K shape: {k.shape}")
    print(f"  Compressed K shape: {k_cmp.shape}")
    print(f"  Input V shape: {v.shape}")
    print(f"  Compressed V shape: {v_cmp.shape}")
    ...
    print("  ✓ TokenCompression test passed!")
    return True
...
def test_nsa_pupil_seg():
    ...
    in_channels = 1
    num_classes = 2
    H, W = 400, 640  # OpenEDS image size

    model = create_nsa_pupil_seg(size="small")
...
def test_combined_loss():
    ...
    spatial_weights = torch.rand(batch_size, H, W)
    dist_map = torch.rand(batch_size, num_classes, H, W)
    alpha = 0.5

    total_loss, ce_loss, dice_loss, surface_loss = criterion(
        logits, target, spatial_weights, dist_map, alpha
    )
...
def test_model_sizes():
    ...
    H, W = 200, 320  # Smaller for faster testing
...
def benchmark_inference():
    ...
    for size in ["tiny", "small", "medium"]:
        model = create_nsa_pupil_seg(size=size).to(device)
        model.eval()
...
def main():
    """Run all tests."""
    print("=" * 60)
    print("NSA Pupil Segmentation Model Tests")
    print("=" * 60)

    tests = [
        ("Token Compression", test_token_compression),
        ("Sliding Window Attention", test_sliding_window_attention),
        ("Spatial NSA", test_spatial_nsa),
        ("NSA Block", test_nsa_block),
        ("NSA Pupil Seg Model", test_nsa_pupil_seg),
